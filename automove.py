...
class AUVRepositioning:
    ...
    def depth_callback(self, depth_msg):
        try:
            ...
            # Determine the direction to move the camera (left or right) along the x-axis
            x_direction = "right" if x_offset >= 0 else "left"

            # Print the information
            print(f"Move the camera {abs(x_offset):.2f} meters {x_direction} along the x-axis")
            print(f"Updated Depth: {depthWhat is PCL in Python?
How to install PCL for Python?
What is PCL in computer vision?
:.2f} meters")

            # Publish the calculated distance_x
            self.publish_distance_x(x_offset)
